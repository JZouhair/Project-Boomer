...
public class controllerress {
	...
	@GetMapping(value="addbook/{libel}/{desc}/{autor}/{quant}/{bloque}")
	public void AddBook(@PathVariable String libel, @PathVariable String desc,
			@PathVariable String autor, @PathVariable int quant, @PathVariable boolean bloque) {
		traitementMetier.AddBook(libel, desc, autor, quant, bloque);
	}
	//bloquer book
	@RequestMapping(value="/bloquerbook", method=RequestMethod.PUT)
	public void bloquerBook(@RequestBody Book book ) {
		traitementMetier.bloquerBook(book);
	}
	//debloquer book
		@RequestMapping(value="/debloquerbook", method=RequestMethod.PUT)
		public void deloquerBook(@RequestBody Book book ) {
			traitementMetier.debloquerBook(book);
		}
